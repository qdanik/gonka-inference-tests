...
| `CUDA error custom_all_reduce.cuh:455 'invalid argument'` on engine init | `--model-extra-args="--disable-custom-all-reduce"` | **mandatory for TP>1** on every supported arch (B200/H200/A100/H100) |
...
| Validator similarity collapses to ~0.6 on `rf_*`/`tool_*` prompts even though executor data is `raw_logprobs` — `validated-by-*.json` shows ~10-15% positions at `logprob: -9999.0` | vLLM's `detect_logprobs_mode()` (vllm/validation.py:51) classifies raw inputs as `processed` when ≥10% of top-K token IDs are <4 — JSON `{`, `"`, etc. naturally cluster low-ID tokens past the threshold. Validator silently switches to processed → schema mask gets applied to top_logprobs → -9999 sentinels at every forced position not in the schema-allowed set. **Fix:** always pass `--logprobs-mode raw_logprobs` (or `processed_logprobs`) explicitly to `e2e validate` AND `e2e infer` — the flag is now pinned per-request body (in addition to server startup), bypassing the heuristic. | every GPU + every image when executor data has JSON/tool/structured-output prompts |
| Same-arch validation gives only sim ≈ 0.83 (e.g. A100→A100, B200→B200), well below expected ≈ 1.0, AND cross-arch metrics depressed by ~10 pp FP across all directions | Inference-side `--logprobs-mode` not pinned per-request body. The saved executor data lands in an inconsistent state that fails even when replayed by an identical server. Both `e2e infer` and `e2e validate` must pass `--logprobs-mode raw_logprobs` (or `processed_logprobs`) explicitly; the validator-side fix alone is insufficient. With the per-request pin in place, same-node A100→A100 gives sim ≈ 0.98 (the residual is marlin's non-determinism, not metric error). | any executor run where `e2e infer` was called without `--logprobs-mode` — re-collect the inference data |
...
2. **vLLM is never exposed publicly.** The container listens only on the remote's loopback (`--network host` → `127.0.0.1:8000`); the framework tunnels through SSH. No `--vllm-url` flag and no inbound firewall rule needed.
