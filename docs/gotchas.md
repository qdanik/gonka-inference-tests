...
| Same-arch validation gives only sim ≈ 0.83 (e.g. A100→A100, B200→B200), well below expected ≈ 1.0, AND cross-arch metrics depressed by ~10 pp FP across all directions | Inference-side `logprobs_mode` mis-pin. If `e2e infer` is called without `--logprobs-mode raw_logprobs` (or with old code that doesn't propagate it per-request), the saved executor data lands in an inconsistent state that fails even when replayed by an identical server. We discovered this 2026-06-07 — same-node A100→A100 gave sim ≈ 0.83 on old data, ≈ 0.98 after re-collecting with the pin. The validator-side fix is NOT enough on its own; both sides need the per-request pin. | every executor run collected before the 2026-06-07 fix was merged — re-collect inference data after upgrading |
